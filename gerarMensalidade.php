...
if (!defined('CAMINHO_INSTALACAO')) {
  include_once('../../includes/defineVariavelAmbiente.php');
}
...
function gera_mensalidade($id_empresa)
{
    ...
    $arr_clientes = buscaClientes($conn, $id_empresa, $mes_ref, $var_talao);
    ...
    if($arr_clientes && !empty($arr_clientes)){
      ...
      $id_cobranca_mensalidade = criaMensalidade($conn, $id_empresa, $tx_descricao_modelo_mensalidade);
      ...
      if($id_cobranca_mensalidade){
        ...
        if(pg_query($conn, $sql_insert)){
          ...
          foreach($arr_clientes as $index => $cliente){
            ...
            $id_venda = 0;
            $ret_venda = inserirVenda($conn, $id_empresa, $cliente, $mes_ref, $dt_venda, $param_dia_util, $feriados, $uf_empresa, $cfop_dentro, $cfop_fora, $id_cobranca_mensalidade, $var_talao, $id_local_estoque);
            echo 'Venda: <pre>';
            print_r($ret_venda);
            echo '</pre>';
            ...
            if($id_venda){
                ...
                $arr_item_venda = buscaItemVenda($conn, $id_empresa, $cliente, $dt_venda, $mes_ref);
                ...
                if($cliente['nfe'] == 3 && $uf_empresa != 'DF'){
                  ...
                  $ret_item = insereItemVendaMunicipal($conn, $id_empresa, $id_venda, $cliente, $arr_item_venda, $id_local_estoque, $cfop, $dt_vcto_parc, $dt_venda);
                ...
                #Adiciona os itens nas vendas padrões
                  ...
                  $ret_item = insereItemVenda($conn, $id_empresa, $id_venda, $cliente, $arr_item_venda, $id_local_estoque, $cfop, $dt_vcto_parc, $dt_venda);
      ...
      if(!count($arr_erro) and count($arr_sucesso)){
        ...
        $var_sql = "INSERT INTO db_gol.tb_passo_mensalidade_automatica (id_empresa, id_cobranca_mensalidade, id_passo, tx_status)
                VALUES ($id_empresa, $id_cobranca_mensalidade, 2, 'EM_PROGRESSO')";

        pg_query($conn, $var_sql);
      ...
      #Chama a index
      $arrayServidor = array('https://'.$_SERVER['SERVER_NAME']."/includes/geraMensalidadeAutomatica/gerarFaturamento.php?id_empresa=$id_empresa&id_cobranca_mensalidade=$id_cobranca_mensalidade");

      if($_SERVER['SERVER_NAME'] == 'srvdsv3.axmsolucoes.com.br'){
        $arrayServidor = array("https://srvdsv3.axmsolucoes.com.br/scriptcase915/app/GOL/includes/geraMensalidadeAutomatica/gerarFaturamento.php?id_empresa=$id_empresa&id_cobranca_mensalidade=$id_cobranca_mensalidade");
      }
...
function buscaClientes($conn, $id_empresa, $mes_ref, $var_talao){
    ...
    foreach($arr_clientes as $index => $cliente){
      ...
      $count_vendas = 0;
      $count_vendas = intval(verificaVendas($conn, $id_empresa, $id_pessoa, $var_talao));
...
function verificaVendas($conn, $id_empresa, $id_pessoa, $var_talao){
...
function criaMensalidade($conn, $id_empresa, $tx_desc_cobranca){
...
function inserirVenda($conn, $id_empresa, $cliente, $mes_ref, $dt_venda, $param_dia_util, $feriados, $uf_empresa, $cfop_dentro, $cfop_fora, $id_cobranca_mensalidade, $var_talao, $id_local_estoque){
...
function buscaItemVenda($conn, $id_empresa, $cliente, $dt_venda, $mes_ref){
...
function insereItemVendaMunicipal($conn, $id_empresa, $id_venda, $cliente, $arr_item_venda, $id_local_estoque, $cfop, $dt_vcto_parc, $dt_venda){
    $vr_total = 0;
    $vr_total_venda = 0;
    $vr_unitario_item = 0;
    $descricao = '';
    ...
    foreach ($arr_item_venda as $item => $detalhe){
      ...
      if($tp_item == 'A'){
        if($param && $param == 'A'){
            $ret_reajuste = verificaRejuste($conn, $id_empresa, $id_pessoa, $id_assinatura, $dt_venda);
        ...
        $arr_desconto = verificaDesconto($conn, $id_empresa, $id_pessoa, $id_assinatura, $dt_venda); 
...
function verificaRejuste($conn, $id_empresa, $id_pessoa, $id_assinatura, $dt_venda){
  $arr_reajuste = array();
    ...
    foreach($pa as $key => $linha){
      ...
      if(($var_mes_ativ == $var_mes_vd) && ($var_ano_vd > $var_ano_ativ) && ($var_vr_reajustado > $var_vr_assinatura)){
        ...
        if($cont && $cont[0]['count'] == 0){
          ...
          $arr_reajuste['msg_reajuste'] = "- <b>$var_descricao_produto R$ ".number_format($var_vr_assinatura,2,',','.')
                          ."</b> houve reajuste de <b>".number_format($var_nu_percentual,4,',','.')." % (".$var_nome_indice.")</b> "
...
function verificaDesconto($conn, $id_empresa, $id_pessoa, $id_assinatura, $dt_venda){
  $arr_desconto = array();
...
function insereItemVenda($conn, $id_empresa, $id_venda, $cliente, $arr_item_venda, $id_local_estoque, $cfop, $dt_vcto_parc, $dt_venda){
  $vr_total = 0;
  $vr_total_venda = 0;
  $arr_msg_reajuste = array();
